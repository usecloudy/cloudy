...
export const updateFolderPublicStatus = async (affectedFolderIds: string[], supabase: SupabaseClient<Database>) => {
	...
	const { data: folderTree } = await supabase.rpc("get_folder_ancestors", {
		folder_ids: affectedFolderIds,
	});
	...
	const updates = await Promise.all(
		folderTree.map(async folder => {
			...
			return {
				id: folder.id,
				access_strategy: hasPublicContent ? FolderAccessStrategies.PUBLIC : FolderAccessStrategies.WORKSPACE,
			};
		}),
	);
